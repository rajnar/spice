    ...
    public function __construct()
    {
     ...
     function login($post)
     {
         $sql="select * from users
                where uname='".$this->db->escape_str($post['uname'])."'
                and password = '".$this->db->escape_str($post['upwd'])."'";
     ...
     {
         $sql = 'select concat(firstname," ",lastname)as name,password from users where email="'.$this->db->escape_str($post['login_name']).'"';
